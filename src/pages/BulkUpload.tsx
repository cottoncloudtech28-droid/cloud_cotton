...
export default function BulkUpload() {
  ...
  useEffect(() => {
    if (!loading && !user) nav(`/auth?redirect=${encodeURIComponent("/admin/bulk")}`);
  }, [user, loading, nav]);
  ...
  const addFiles = async (files: FileList | null) => {
    ...
    for (const f of Array.from(files)) {
      ...
      next.push({
        ...
        stock: "10",
        colorsText: "",
        bgPrompt: PRESET_BACKGROUNDS[0].value,
        anglePrompt: "",
      });
    }
    setRows((r) => [...r, ...next]);
  ...
  const runEdit = async (row: Row, prompt: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("ai-image-edit", {
        body: { image_base64: row.currentDataUrl, prompt },
      });
  ...
  const resetImage = (row: Row) =>
    update(row.id, { currentDataUrl: row.originalDataUrl });
  ...
  const saveAll = async () => {
    ...
    for (const r of rows) {
      try {
        ...
        const path = `${user.id}/bulk-${Date.now()}-${r.id}.${ext}`;
        const { error: upErr } = await supabase.storage.from("products").upload(path, blob, { contentType: blob.type });
        ...
        const { error: insErr } = await supabase.from("products").insert([{
          ...
          category: r.category.trim() || "stationery",
      ...
      <div className="min-h-screen flex w-full">
        ...
        <div className="flex items-end justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-2">
            <Button variant="outline" className="rounded-full" onClick={() => nav("/admin")}>Back to Admin</Button>
            <Button
              disabled={saving || rows.length === 0}
              onClick={saveAll}
              className="bg-gradient-primary text-primary-foreground border-0 rounded-full"
            >
                ...
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        placeholder="pink, lilac, mint"
                        value={r.colorsText}
                        onChange={(e) => update(r.id, { colorsText: e.target.value })}
                      />
                    </div>
                    <div className="sm:col-span-2">
                  ...
                  <div className="rounded-2xl bg-muted/40 p-3 space-y-3">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <Wand2 className="h-4 w-4" /> AI image editing
                    </div>
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mb-2">
                        {PRESET_BACKGROUNDS.map((p) => (
                          <Badge
                            ...
                            onClick={() => update(r.id, { bgPrompt: p.value })}
                          >
                            {p.label}
                          </Badge>
                        ))}
                      ...
                      <div className="flex flex-wrap gap-1 mb-2">
                        {PRESET_ANGLES.map((p) => (
                          <Badge
                            ...
                            variant="outline"
                            className="cursor-pointer hover:bg-primary/10"
                            onClick={() => update(r.id, { anglePrompt: p.value })}
                          >
                            {p.label}
                          </Badge>
                        ))}
