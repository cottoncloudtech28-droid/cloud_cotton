...
import {
  ShoppingBag, Heart, Minus, Plus, Truck, RotateCcw, Shield, ChevronRight,
} from "lucide-react";
    ...
    apiFetch(`/api/products/${id}`)
      .then(async (p: Product) => {
        ...
        const [rel, wids] = await Promise.allSettled([
          apiFetch(`/api/products?cat=${encodeURIComponent(p.category)}`),
          user ? getWishlistIds() : Promise.resolve([]),
        ]);
        if (rel.status === "fulfilled")
          setRelated((rel.value as Product[]).filter((x) => x.id !== id).slice(0, 4));
